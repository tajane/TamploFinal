...
public class AddActionIntoDayPlanTest extends TestBase
{
	...
	@Test
	public void AddActionDayPlanTest() throws InterruptedException 
	{
		...
		dayplanobject.clickOnAddButtonOfFirstActionFromDue();
		
		test.log(Status.PASS, MarkupHelper.createLabel("First Action Successfully added into selected date day plan", ExtentColor.GREEN));
		
		int openactioncountaftermoving = dayplanobject.getOpenActionCountFromDueFilter();
		
		test.log(Status.PASS, MarkupHelper.createLabel("open action count after moving from due action filter : " + openactioncountaftermoving, ExtentColor.GREEN));
		
		int dayplanopenactioncountaftermoving = dayplanobject.getOpenActionCountFromDayPlan();
		
		test.log(Status.PASS, MarkupHelper.createLabel("day plan open action count after moving into day plan  is : " + dayplanopenactioncountaftermoving, ExtentColor.GREEN));
	}
